...
public class Collision {
	...
	/*
	 * CollideWithBlock checkt ob der currentBlock mit einem der bereits gezeichneten Blöcke in der Map Variable kollidiert und verhindert das Bewegen falls die Kollision true ist.
	 ...
	 * Es wird bei jeder Kollision geschaut ob der Block im Spielfeld ist, da es sonst Probleme mit dem Map Array geben würde (diese beinhaltet ja nur die map)
	 * 
	 */
							...
							for (int j = 0; j < b.getBounds()[b.getRotation()][i].length; j++) {// 2. der beiden Schleifen, die jedes einzelne Viereck des Blockes durchläuft (y)
								if (b.getBounds()[b.getRotation()][i][j] == 1) { //Nur wahr, wenn das Viereck des Blockes ausgefüllt ist. "Luft Teile" des Blcokes werden ignoriert
									...
									if (Game.map[b.getX() + i][b.getY() + j + 1] >= 1) { // Der Block wird "fikitv" eins weiter nach unten gesetzt, es wird geschaut ob er mit einem Block unterhalb überlappt
										...
										Game.spawnNewBlock = true; // Falls der Block mit einem Block unterhalb kollidiert wird das Spawnen eines neuen Blockes af true gesetzt
 ...
 * Es wird bei jeder Kollision geschaut ob der Block im Spielfeld ist, da es sonst Probleme mit dem Map Array geben würde (diese beinhaltet ja nur die map)
 * Der Block wird immer mit der neuen Rotation rot auf Kollision überprüft
 */
	...
	/*
	 * die Methode collideWithWall überprüft ob eine Kollision mit den außen Wänden vorliegt und gibt einen boolean zurück
	 * Die Direction -1 checkt die Kollision mit der Wand nach links a
	 * Die Direction 0 checkt eine Kollision mit der unteren Grenze (boden der Map)
	 * die Direction 1 checkt die Kollision mit der rechten Wand 
	 */
...
/*
 * Methode die den Block fals diese unten angekommen ist in das 2-dimensional Maparray überträgt. Der Block b wird dabei mitübergeben  	
 */
	...
	/*
	 * Methode zum Überprüfen der vollständigkeit der Reihen, damit diese leer gesetzt werden und die Punkte gezählt werden können
	 */
	...
	*/
	private static void checkLoose() { 
		for (int x = 0; x < Game.map.length; x++) { // Durchlaufen aller x-Werte der obersten Reihe
			...
			if (Game.map[x][0] > 0) { // Falls in der obersten Reihe( außerhalb des eigentlichen Spielfelds)ein Block ist wird dies ausgeführt
